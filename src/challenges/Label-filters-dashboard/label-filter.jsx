...
const LabelFilter = () => {
  ...
  const handleToggle = (animalClass) => {
    if (selectedClass.includes(animalClass)) {
      setselectedClass(
        selectedClass.filter((selected) => selected !== animalClass)
      );
    } else {
      setselectedClass([...selectedClass, animalClass]);
    }
  };

  const filteredAnimals = animalData.filter((data) => {
    return selectedClass.length === 0
      ? true
      : selectedClass.includes(data.class);
  });
      ...
      <div data-testid="labels-wrapper-id" className="label-container">
        {animalClasses.map((animalClass) => (
          <div
            ...
            className="label"
            key={animalClass}
            onClick={() => handleToggle(animalClass)}
            style={{
              backgroundColor: selectedClass.includes(animalClass) && "#333",
              color: selectedClass.includes(animalClass) && "#fff",
            }}
          >
            {animalClass}
          </div>
